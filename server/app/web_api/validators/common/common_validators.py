...
def validate_email(email: str | None, max_length: int | None = None) -> ValidationResult:
    if not email:
...
def validate_password(password: str | None) -> ValidationResult:
    if not password:
...
def validate_text(value: str | None, field_name: str, min_length: int = 1, max_length: int | None = None) -> ValidationResult:
    if value is None:
...
def validate_number(value: int | float | None, field_name: str, min_value: int | float | None = None, max_value: int | float | None = None) -> ValidationResult:
    if value is None:
        return ValidationResult.fail(f"{field_name} is required")
...
def validate_date(value: date | None, field_name: str, allow_future: bool = False) -> ValidationResult:
...
def validate_enum(value: str | int | Enum | None, enum_type: type[Enum], field_name: str) -> ValidationResult:
    if value is None:
        return ValidationResult.fail(f"{field_name} is required")
...
def validate_image(img_b64: str, field_name: str, allowed_mime_types: set[str] | None = None, max_size_bytes: int | None = None) -> ValidationResult:
    ...
    if allowed_mime_types:
        mime_type = Image.MIME.get(img.format) if img.format else None

...
class GatewayFlightsController:
    def __init__(self, gateway_airline_service: IGatewayAirlineService) -> None:
        self._flights_blueprint = Blueprint('flights', __name__, url_prefix='/api/v1/flights')
        self.gateway_airline_service = gateway_airline_service
        self._register_routes()
    
    def _register_routes(self) -> None:
        self._flights_blueprint.add_url_rule('/airlines', view_func=self.create_airline, methods=['POST'])
        self._flights_blueprint.add_url_rule('/airlines', view_func=self.get_all_airlines, methods=['GET'])
        self._flights_blueprint.add_url_rule('/airlines/<int:airline_id>', view_func=self.get_airline, methods=['GET'])
        self._flights_blueprint.add_url_rule('/airlines/<int:airline_id>', view_func=self.update_airline, methods=['PATCH'])
        self._flights_blueprint.add_url_rule('/airlines/<int:airline_id>', view_func=self.delete_airline, methods=['DELETE'])
    ...
    def blueprint(self) -> Blueprint:
        return self._flights_blueprint